...
import tensorflow.compat.v1 as tf  # 替换原版 `import tensorflow as tf`
tf.disable_v2_behavior()          # 禁用 TensorFlow 2.x 的默认行为
...
cdef class py_PrepareBatchGraph:
    ...
    def SetupTrain(self,idxes,g_list,covered,list actions):
        ...
        for _g in g_list:
            ...
            deref(inner_Graph).in_nodes_weight = _g.in_nodes_weight
            deref(inner_Graph).out_nodes_weight = _g.out_nodes_weight
            deref(inner_Graph).betweenness = _g.betweenness
            deref(inner_Graph).in_total_nodes_weight=_g.in_total_nodes_weight
            deref(inner_Graph).out_total_nodes_weight=_g.out_total_nodes_weight
            deref(inner_Graph).edge_weights = _g.edge_weights
            deref(inner_Graph).in_edge_weight_sum = _g.in_edge_weight_sum
            deref(inner_Graph).edge_weight_sum = _g.edge_weight_sum
