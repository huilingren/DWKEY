...
cdef class py_NStepReplayMem:
    ...
    def Add(self,mvcenv,int nstep):
        ...
        deref(self.inner_Graph).in_nodes_weight=g.in_nodes_weight
        deref(self.inner_Graph).out_nodes_weight=g.out_nodes_weight
        deref(self.inner_Graph).betweenness=g.betweenness
        deref(self.inner_Graph).in_total_nodes_weight=g.in_total_nodes_weight
        deref(self.inner_Graph).out_total_nodes_weight=g.out_total_nodes_weight
        deref(self.inner_Graph).total_betweenness=g.total_betweenness
    ...
    def GenNetwork(self, g):    #networkx2four
        ...
        for node in nodes:
            in_weights.append(g.nodes[node]['in_degree'])  # 访问节点的入度中心性
            out_weights.append(g.nodes[node]['out_degree'])  # 访问节点的出度中心性
