'''
file:graph.pxd 类graph的定义文件对应graph.h
'''
#Cython已经编译了C++的std模板库，位置在~/Cython/Includes/lincpp/
from libcpp.vector cimport vector
from libcpp.memory cimport shared_ptr
from libcpp.map cimport map
from libcpp.pair cimport pair
cdef extern from "./src/lib/graph.h":
    cdef cppclass Graph:
        Graph()except+
        Graph(const int _num_nodes, const int _num_edges, const int* edges_from, const int* edges_to,const double* _in_nodes_weight,const double* _out_nodes_weight,const double* _betweenness,const double* _edge_weights) except+
        int num_nodes
        int num_edges
        vector[vector[int]] adj_list
        vector[pair[int,int]] edge_list
        vector[double] in_nodes_weight
        vector[double] out_nodes_weight
        vector[double] betweenness
        vector[double] edge_weights
        double in_total_nodes_weight
        double out_total_nodes_weight
        double total_betweenness
        double edgetotalweight
        vector[double] edge_weight_sum
        vector[double] in_edge_weight_sum

cdef extern from "./src/lib/graph.h":
    cdef cppclass GSet:
        GSet()except+
        void InsertGraph(int gid, shared_ptr[Graph] graph)except+
        shared_ptr[Graph] Sample()except+
        shared_ptr[Graph] Get(int gid)except+
        void Clear()except+
        map[int, shared_ptr[Graph]] graph_pool

