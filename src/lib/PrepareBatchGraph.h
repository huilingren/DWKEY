...
public:
    ...
    int GetStatusInfoDebug(std::shared_ptr<Graph> g, int num, const int* covered, int& counter,int& twohop_number,int& threehop_number, std::vector<int>& idx_map);
    int getMaxConnectedNodesNum(std::shared_ptr<Graph> g,const std::vector<int>& covered_vec);
